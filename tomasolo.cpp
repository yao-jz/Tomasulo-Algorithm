...
string unitname[NUMUNITS] = {"LOAD1", "LOAD2", "STORE1", "STORE2", "INT1", "INT2"}; /* 执行单元的名称 */
...
string statename[4] = {"ISSUING", "EXECUTING", "WRITINGRESULT", "COMMTITTING"}; /*  状态名称 */
...
void printState(machineState *statePtr, int memorySize)
{
  ...
  for (i = 0; i < NUMUNITS; i++)
  {
    if (statePtr->reservation[i].busy == 1)
    {
      ...
      {
        printf("Qj = '%s' ", unitname[statePtr->reservation[i].Qj - 1].c_str());
      }
      ...
      {
        printf("Qk = '%s' ", unitname[statePtr->reservation[i].Qk - 1].c_str());
      }
      printf(" ExTimeLeft = %d  RBNum = %d instr = %d\n",
             statePtr->reservation[i].exTimeLeft,
             statePtr->reservation[i].reorderNum,statePtr->reservation[i].instr);
    }
  }
  ...
  for (i = 0; i < RBSIZE; i++)
  {
    if (statePtr->reorderBuf[i].busy == 1)
    {
      ...
      printf("instr %d  executionUnit '%s'  state %s  valid %d  result %d storeAddress %d\n",
             ...
             unitname[statePtr->reorderBuf[i].instr, statePtr->reorderBuf[i].execUnit].c_str(),
             statename[statePtr->reorderBuf[i].instrStatus].c_str(),
...
void issueInstr(int instr, int unit, machineState *statePtr, int reorderNum)
{
   ...
   * 填写保留栈和ROB项的内容.
   * 注意, 要在所有的字段中写入正确的值.
   * 检查寄存器状态, 相应的在Vj,Vk和Qj,Qk字段中设置正确的值: f
   * 对于I类型指令, 设置Qk=0,Vk=0; f
   * 对于sw指令, 如果寄存器有效, 将寄存器中的内存基地址保存在Vj中; f
   * 对于beqz和j指令, 将当前PC+1的值保存在Vk字段中. f
   * 如果指令在提交时会修改寄存器的值, 还需要在这里更新寄存器状态数据结构. f
   */
  int op = opcode(instr);
  ...
  if (op == 43) { // store
    int rs1 = field0(instr); // r2
    int rs2 = field1(instr); // r1
    if (statePtr->regResult[rs1].valid == 0) { // 还没算完
      if (statePtr->reorderBuf[statePtr->regResult[rs1].reorderNum].valid==1) { // 已经在ROB中计算完成
        statePtr->reservation[unit].Vj = statePtr->reorderBuf[statePtr->regResult[rs1].reorderNum].result;
        statePtr->reservation[unit].Qj = 0;
      } else { // 还未计算完成
        statePtr->reservation[unit].Vj = 0;
        statePtr->reservation[unit].Qj = statePtr->reorderBuf[statePtr->regResult[rs1].reorderNum].execUnit;
      }
    } else { // 已经算完了，可以加载数据
      statePtr->reservation[unit].Vj = statePtr->regFile[rs1];
      statePtr->reservation[unit].Qj = 0;
    }
    if (statePtr->regResult[rs2].valid == 0) { // 还没算完
      if (statePtr->reorderBuf[statePtr->regResult[rs2].reorderNum].valid==1){
        statePtr->reservation[unit].Vk = statePtr->reorderBuf[statePtr->regResult[rs2].reorderNum].result;
        statePtr->reservation[unit].Qk = 0;
      } else {
        statePtr->reservation[unit].Vk = 0;
        statePtr->reservation[unit].Qk = statePtr->reorderBuf[statePtr->regResult[rs2].reorderNum].execUnit;
      }
    } else { //已经算完了，可以加载数据
      statePtr->reservation[unit].Vk = statePtr->regFile[rs2];
      statePtr->reservation[unit].Qk = 0;
    }
  } else if (op == regRegALU) { // R
  ...
  statePtr->reorderBuf[reorderNum].pc = statePtr->pc;
  statePtr->reorderBuf[reorderNum].execUnit= unit;
  statePtr->reorderBuf[reorderNum].valid = 0;
...
int checkReorder(machineState *statePtr, int *headRB, int *tailRB)
{
  ...
  if (statePtr->reorderBuf[(*tailRB+1)%16].busy==0){
    *tailRB = (*tailRB + 1)%16;
    return *tailRB;
  }
  return -1;
...
{
  int op = opcode(rStation.instr), immed = immediate(rStation.instr), function = func(rStation.instr), address = jumpAddr(rStation.instr);
  /*
   * [TODO]
   * 这个函数负责计算有输出的指令的结果.
   * 你需要完成下面的case语句....
...
inline void clearMachine(machineState *statePtr)
{
...
int main(int argc, char *argv[])
{
  ...
  { /* 执行循环:你应该在执行halt指令时跳出这个循环 */
    ...
    if (statePtr->reorderBuf[headRB].instrStatus == COMMITTING && headRB != -1 && statePtr->reorderBuf[headRB].busy) {
      int op = opcode(statePtr->reorderBuf[headRB].instr);
      if (op == SW) {  
        statePtr->memory[statePtr->reorderBuf[headRB].storeAddress] = statePtr->reorderBuf[headRB].result;
      }
      else if (op == J) {
        statePtr->pc = statePtr->reorderBuf[headRB].pc + 1 + jumpAddr(statePtr->reorderBuf[headRB].instr);
        headRB = -1;
        tailRB = -1;
        clearMachine(statePtr);
      } else if (op == BEQZ && statePtr->reorderBuf[headRB].result == 0) {
        statePtr->pc = statePtr->reorderBuf[headRB].pc + 1 + immediate(statePtr->reorderBuf[headRB].instr);
        headRB = -1;
        tailRB = -1;
        clearMachine(statePtr);
      } else if (op == HALT) { // halt, stop machine
        break;
      }
      for (int i = 0; i < NUMREGS; i++) { // 更新寄存器
        if(statePtr->regResult[i].valid == 0 && statePtr->regResult[i].reorderNum == headRB) {
          statePtr->regResult[i].valid = 1;
          statePtr->regFile[i] = statePtr->reorderBuf[headRB].result;
        }
      }
      statePtr->reorderBuf[headRB].valid = 0;
    ...
    if (headRB != -1) {
      for(int i = 0; i < RBSIZE; i++) {
        int real_i = (headRB + i) % RBSIZE;
        if (statePtr->reorderBuf[real_i].busy == 0) continue; // 空闲直接继续
        int status = statePtr->reorderBuf[real_i].instrStatus;
        if (status == WRITINGRESULT) {
          statePtr->reorderBuf[real_i].valid = 1;
          int result = getResult(statePtr->reservation[statePtr->reorderBuf[real_i].execUnit],statePtr);
          statePtr->reorderBuf[real_i].result = result;
          updateRes(statePtr->reorderBuf[real_i].execUnit,statePtr,result);
          if (opcode(statePtr->reorderBuf[real_i].instr) == 43) { // store, 写好地址
            statePtr->reorderBuf[real_i].storeAddress = statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].Vj + immediate(statePtr->reorderBuf[real_i].instr);
          }
          statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].busy = 0;
          statePtr->reorderBuf[real_i].instrStatus = COMMITTING;
        } else if (status == EXECUTING) {
          statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].exTimeLeft -=1;
          if (statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].exTimeLeft == 0) {
            statePtr->reorderBuf[real_i].instrStatus =WRITINGRESULT;
          }
        } else if (status == ISSUING) {
          if (statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].Qj == 0 && statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].Qk == 0) {
            statePtr->reorderBuf[real_i].instrStatus = EXECUTING;
            int op = opcode(statePtr->reorderBuf[real_i].instr);
            if (op == LW || op == SW) statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].exTimeLeft = 2;
            else if (op == J || op == BEQZ) statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].exTimeLeft = 3;
            else statePtr->reservation[statePtr->reorderBuf[real_i].execUnit].exTimeLeft = 1;
          }
        }
      }
    }
     ...
     * 如果也能找到空闲空间, 发射指令.
     */
    int freeResStation = -1;
    int op = opcode(statePtr->memory[statePtr->pc]);
    int begin = 0, end = 0;
    if(op == LW) {
      begin = 0;
      end = 2;
    ...
    if (freeResStation != -1) {
      int freeReorder = checkReorder(statePtr, &headRB, &tailRB);
      if (freeReorder != -1) {
        issueInstr(statePtr->memory[statePtr->pc],freeResStation,statePtr, freeReorder);
        statePtr->pc += 1;
      }
    }
